...
export default class HeterogeneousAgentCtr extends ControllerModule {
  ...
  // ─── AskUserQuestion MCP server (Claude Code AskUserQuestion MCP server with custom UI intervention: local HTTP Streamable MCP server exposes ask_user_question tool with 5min blocking + progress keepalive) ───
  ...
  private async setupInterventionForOp(
    ...
    // (no ToolSearch hop). (Claude Code AskUserQuestion MCP server with custom UI intervention: local HTTP Streamable MCP server exposes ask_user_question tool with 5min blocking + progress keepalive) — falls back to the
