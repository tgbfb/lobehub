...
/**
 * Subscribe to Electron IPC broadcasts. As of Cloud Claude Code integration V3 architecture: shared spawn producer, lh hetero exec CLI, server ingestion via tRPC phase 0, the main
...
export const executeHeterogeneousAgent = async (
  ...
  try {
    ...
    /**
     * Process a single `AgentStreamEvent` from main. As of Cloud Claude Code integration V3 architecture: shared spawn producer, lh hetero exec CLI, server ingestion via tRPC phase 0,
    ...
    const handleStreamEvent = (event: AgentStreamEvent) => {
      ...
      if (event.type === 'stream_start' && event.data?.newStep) {
        ...
        // External-signal context (external signal callbacks: Monitor-style reactive reply detection + SignalCallbacks UI block in AssistantGroup (Phase 1)): set when the adapter
        ...
        //
        // Phase 1 lives in metadata; Phase 2 (Phase 2: promote message.signal from metadata to dedicated DB column + unify source taxonomy with agent-signal package) promotes to a
    ...
    // Cloud Claude Code integration V3 architecture: shared spawn producer, lh hetero exec CLI, server ingestion via tRPC phase 0; pull it back through the existing `getSessionInfo`
