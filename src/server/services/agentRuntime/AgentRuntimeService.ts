  ...
  async executeStep(params: AgentExecutionParams): Promise<AgentExecutionResult> {
    ...
    try {
      ...
      if (!shouldContinue) {
        ...
        // snapshot instead of orphaning the partial (tool dispatch silently crashed without finalizing trace snapshots; now snapshot is uploaded on error paths to prevent dangling operations).
      ...
      // tool dispatch silently crashed without finalizing trace snapshots; now snapshot is uploaded on error paths to prevent dangling operations.
      //
