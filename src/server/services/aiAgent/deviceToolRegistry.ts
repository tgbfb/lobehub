 ...
 *      helper closes the activator bypass documented in converge device-access check pipeline + fix activator bypass that exposed owner device list to external bot senders (an
 ...
 * `allowExplicitActivation` (converge device-access check pipeline + fix activator bypass that exposed owner device list to external bot senders), so the **physical** filter
