...
// Default state mocks for the single-human thread relaxation: bot responds to non-mention messages in channel threads with only one human participant, auto-announces mention-only mode when second human joins participant tracking. Tests that
...
// keep their pre- the fix for watch-keyword trigger: bot responds to operator-configured keywords in channel messages without requiring @mention behaviour. Individual tests can replace this via
...
vi.mock('../platforms', () => ({
  ...
  // watch-keyword trigger: bot responds to operator-configured keywords in channel messages without requiring @mention: mirror the production helpers just well enough that
...
describe('BotMessageRouter', () => {
  beforeEach(() => {
    ...
    // single-human thread relaxation: bot responds to non-mention messages in channel threads with only one human participant, auto-announces mention-only mode when second human joins participant tracking — restore defaults wiped by
  ...
  describe('onSubscribedMessage policy', () => {
    // single-human thread relaxation: bot responds to non-mention messages in channel threads with only one human participant, auto-announces mention-only mode when second human joins introduced single-human thread relaxation: a non-mention
    ...
    it('should skip non-mention messages in a multi-human group thread', async () => {
      // single-human thread relaxation: bot responds to non-mention messages in channel threads with only one human participant, auto-announces mention-only mode when second human joins: post-fix the gate keys off thread.isDM || mention ||
    ...
    it('should respond to non-mention messages while the channel thread is still single-human (single-human thread relaxation: bot responds to non-mention messages in channel )', async () => {
    ...
    it('should announce mention-only mode once when a second human joins (single-human thread relaxation: bot responds to non-mention messages in channel )', async () => {
    ...
    it('should wake on a watch-keyword match in a subscribed group thread (watch-keyword trigger: bot responds to operator-configured keywords in channel m)', async () => {
      const handler = await loadSubscribedHandler({
        dmPolicy: 'open',
    ...
    it('should not change behaviour when watchKeywords is empty/missing', async () => {
      // Sanity: existing call sites must keep their pre- the fix for watch-keyword trigger: bot responds to operator-configured keywords in channel messages without requiring @mention semantics.
  ...
  describe('onNewMessage DM catch-all', () => {
    ...
    // ---- watch-keyword trigger: bot responds to operator-configured keywords in channel messages without requiring @mention: channel-side keyword wake via catch-all ----
    //
