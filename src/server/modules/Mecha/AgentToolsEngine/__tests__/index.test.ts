...
describe('createServerToolsEngine', () => {
  ...
  it('drops device manifests from every source when excludeIdentifiers is set (converge device-access check pipeline + fix activator bypass that exposed owner )', () => {
...
describe('createServerAgentToolsEngine', () => {
  ...
  describe('RemoteDevice tool enable rules', () => {
    ...
    it('should enable RemoteDevice in bot conversations when caller is trusted (canUseDevice=true)', () => {
      // The `!isBotConversation` clause was dropped in gate device tools by sender identity for bot invocations: external senders can't access owner's local/remote device tools (canUseDevice policy) — the
  ...
  describe('canUseDevice gate (gate device tools by sender identity for bot invocations: external senders can't device access policy)', () => {
