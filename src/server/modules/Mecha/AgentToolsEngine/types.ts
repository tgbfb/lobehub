...
export interface ServerAgentToolsEngineConfig {
   ...
   * resolve `lobe-remote-device` via the activator (converge device-access check pipeline + fix activator bypass that exposed owner device list to external bot senders).
   */
   ...
   * This is the final post-merge wall referenced in converge device-access check pipeline + fix activator bypass that exposed owner device list to external bot senders.
   */
