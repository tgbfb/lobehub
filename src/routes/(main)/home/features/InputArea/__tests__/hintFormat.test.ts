...
describe('stripMarkdownLinks', () => {
  ...
  it('replaces multiple markdown links in one string', () => {
    expect(
      stripMarkdownLinks(
        '帮我整理 [2.1 改版](/agent/inbox/tpc_a) 和 [Cloud Claude Code integration V3 architecture: shared spawn producer, lh hetero exec CLI, server ingestion via tRPC](/task/T-1) 的下一步...',
      ),
    ).toBe('帮我整理 2.1 改版 和 Cloud Claude Code integration V3 architecture: shared spawn producer, lh hetero exec CLI, server ingestion via tRPC 的下一步...');
  });
