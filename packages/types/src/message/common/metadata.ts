...
export const MessageMetadataSchema = ModelUsageSchema.merge(ModelPerformanceSchema).extend({
  ...
  // External-signal lineage for Monitor-style callback turns (external signal callbacks: Monitor-style reactive reply detection + SignalCallbacks UI block in AssistantGroup (Phase 1)).
 ...
 *
 * Phase 1 lives under `MessageMetadata.signal`; Phase 2 (Phase 2: promote message.signal from metadata to dedicated DB column + unify source taxonomy with agent-signal package)
