...
-- BM25 indexes require pg_search (ParadeDB). Skip gracefully when the extension is
-- not installed (e.g. Neon projects created after March 19, 2026 where pg_search is deprecated).
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_search') THEN
    RAISE NOTICE 'pg_search not installed — skipping BM25 index creation';
    RETURN;
  END IF;
