...
export class FlatListBuilder {
  ...
  private buildFlatListRecursive(
    ...
    for (const childId of children) {
      ...
      if (message.role === 'assistant' && message.tools && message.tools.length > 0) {
        ...
        // Gather external-signal callback blocks (external signal callbacks: Monitor-style reactive reply detection + SignalCallbacks UI block in AssistantGroup (Phase 1)) for any
        ...
        // Post-task-summary turns (external signal callbacks: Monitor-style reactive reply detection + SignalCallbacks UI block in AssistantGroup (Phase 1)) — toolless siblings of
  ...
  private createAssistantGroupMessage(
    ...
    // (external signal callbacks: Monitor-style reactive reply detection + SignalCallbacks UI block in AssistantGroup (Phase 1)) so AssistantGroupMessage can render `<SignalCallbacks>`
    ...
    // Snapshot post-task-summary turns as content blocks (external signal callbacks: Monitor-style reactive reply detection + SignalCallbacks UI block in AssistantGroup (Phase 1)).
