 ...
 * structural type.
 *
 * Phase 2 (Phase 2: promote message.signal from metadata to dedicated DB column + unify source taxonomy with agent-signal package) promotes this to a dedicated `messages.signal`
 ...
 *
 * Phase 2 compat seam (Phase 2: promote message.signal from metadata to dedicated DB column + unify source taxonomy with agent-signal package): when the `messages.signal` column
...
export class MessageCollector {
  ...
  collectAssistantChain(
    ...
    for (const toolMsg of toolMessages) {
      ...
      for (const nextMsg of nextMessages) {
        ...
        // Skip signal-tagged toolless callbacks (external signal callbacks: Monitor-style reactive reply detection + SignalCallbacks UI block in AssistantGroup (Phase 1)) — they're a
  ...
  /**
   * Flat-list variant — find post-task-summary assistants (external signal callbacks: Monitor-style reactive reply detection + SignalCallbacks UI block in AssistantGroup (Phase 1)),
  ...
  collectAssistantGroupMessages(
    ...
    for (const toolNode of idNode.children) {
      ...
      // toolless siblings (Monitor callbacks etc., external signal callbacks: Monitor-style reactive reply detection + SignalCallbacks UI block in AssistantGroup (Phase 1)) share the
  ...
  /**
   * Collect post-task-summary toolless siblings (external signal callbacks: Monitor-style reactive reply detection + SignalCallbacks UI block in AssistantGroup (Phase 1)) — assistants
