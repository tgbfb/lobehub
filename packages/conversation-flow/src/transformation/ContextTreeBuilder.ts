...
export class ContextTreeBuilder {
  ...
  private createAssistantGroupNode(message: Message, idNode: IdNode): AssistantGroupNode {
    ...
    // Append external-signal callback blocks (external signal callbacks: Monitor-style reactive reply detection + SignalCallbacks UI block in AssistantGroup (Phase 1)) at the END of
    ...
    // (external signal callbacks: Monitor-style reactive reply detection + SignalCallbacks UI block in AssistantGroup (Phase 1)) — toolless assistants tagged with
