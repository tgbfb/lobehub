...
export class ClaudeCodeAdapter implements AgentEventAdapter {
  ...
  private handleSystem(raw: any): HeterogeneousAgentEvent[] {
    // CC's long-running task lifecycle (Monitor, etc., external signal callbacks: Monitor-style reactive reply detection + SignalCallbacks UI block in AssistantGroup (Phase 1)).
  ...
  private openMainMessage(
    ...
    this.currentMessageId = messageId;
    this.stepIndex++;
    // Signal-callback detection (external signal callbacks: Monitor-style reactive reply detection + SignalCallbacks UI block in AssistantGroup (Phase 1)): if this turn opened
