 ...
 * — (max_completion=0 / input exceeding context window: harness clamped to 0 but still sent requests, failing fast with context_exceeded_pre_flight instead) of failing fast instead of issuing a
 ...
 * that the upstream would happily serve. (max_completion=0 / input exceeding context window: harness clamped to 0 but still sent requests, failing fast with context_exceeded_pre_flight instead).
 */
