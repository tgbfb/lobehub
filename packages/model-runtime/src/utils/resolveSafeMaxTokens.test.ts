...
describe('assertContextWithinWindow', () => {
  ...
  it('attaches max_completion=0 / input exceeding context window: harness clamped to 0 but stil structured payload via toPayload()', () => {
    const longContent = 'a'.repeat(20_000);
    try {
      assertContextWithinWindow(
        { messages: [{ content: longContent, role: 'user' }], model: 'tight-model' } as any,
  ...
  it('does NOT reject a near-limit prompt that still fits within the window', () => {
    // Regression for: max_completion=0 / input exceeding context window: harness clamped to 0 but still sent requests, failing fast with context_exceeded_pre_flight instead PR review feedback: the helper was
