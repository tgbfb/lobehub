...
export interface OpenAICompatibleFactoryOptions<T extends Record<string, any> = any> {
  ...
  chatCompletion?: {
     ...
     * — (max_completion=0 / input exceeding context window: harness clamped to 0 but still sent requests, failing fast with context_exceeded_pre_flight instead).
     *
...
export const createOpenAICompatibleRuntime = <T extends Record<string, any> = any>({
  ...
  return class LobeOpenAICompatibleAI implements LobeRuntimeAI {
    ...
    protected handleError(error: any): ChatCompletionErrorPayload {
      ...
      // raw provider 400. (max_completion=0 / input exceeding context window: harness clamped to 0 but still sent requests, failing fast with context_exceeded_pre_flight instead).
