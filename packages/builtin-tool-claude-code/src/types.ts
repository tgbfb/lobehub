...
export enum ClaudeCodeApiName {
   ...
   * intervention UI for CC's clarifying-question flow (Claude Code AskUserQuestion MCP server with custom UI intervention: local HTTP Streamable MCP server exposes ask_user_question tool with 5min blocking + progress keepalive); not
   ...
   * terminates it — (external signal callbacks: Monitor-style reactive reply detection + SignalCallbacks UI block in AssistantGroup (Phase 1))in the adapter). Rendered by a dedicated
 ...
 * registers it and `system task_notification` ends it ((external signal callbacks: Monitor-style reactive reply detection + SignalCallbacks UI block in AssistantGroup (Phase 1))in the
