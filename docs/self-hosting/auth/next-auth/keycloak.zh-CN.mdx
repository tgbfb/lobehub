...
<Steps>
  ...
  | `NEXT_AUTH_SSO_PROVIDERS` | 必选 | 选择 LobeHub 的单点登录提供商。使用 Keycloak 请填写 `keycloak`。                                                  |
  ...
  <Callout type={'warning'}>
    更新环境变量后，如果你使用的是 Docker，必须**删除并重新创建**容器，仅重启容器不会使新的环境变量生效。请执行 `docker rm
    <容器名>` 再重新启动（或使用 Docker Compose 时执行 `docker compose down && docker compose up -d`）。若登录页面未显示
    Keycloak 登录选项，这是最常见的原因。
  </Callout>
</Steps>

<Callout type={'info'}>
  部署成功后，用户将可以通过 Keycloak 身份认证并使用 LobeHub。若登录页面未显示 Keycloak 登录选项，请确认已设置
  `NEXT_AUTH_SSO_PROVIDERS=keycloak`，并确保在修改环境变量后已完全重新创建容器。
</Callout>
